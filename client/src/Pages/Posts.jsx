  ...
  return (
    <div className={"flex flex-col h-screen justify-between"}>
      ...
      <section className="latest max-w-7xl mx-auto mt-20">
        ...
        <h2
          data-aos={"zoom-in"}
          className={"text-gray-600 font-bold text-xl text-center mt-6 mb-2"}
        >
          Ημερομηνία
        </h2>
        <div
            ...
            <div key={`anakoinwsi${0}`} data-aos={"zoom-in"}>
              {currentMonth && currentYear && !isLoading && (
                <h1 className={"text-2xl text-center"}>
                  Δεν υπάρχουν ανακοινώσεις για {currentMonth.name} -{" "}
                  {currentYear.name}
                </h1>
              )}
            </div>
        ...
        {Object.keys(allPosts).length > 0 &&
          ...
          allPosts[currentYear.name][currentMonth.name].length === 1 && (
            <div className="mb-2">
              ...
              >
                <PostCard
                  props={allPosts[currentYear.name][currentMonth.name][0]}
                />
              </div>
            </div>
          )}
